  ...
  return (
    <button
      ...
      className={`
        ...
        ${isSelected && !disabled ? 'scale-[1.01]' : !disabled ? 'hover:scale-[1.005]' : ''}
      `}
    >
      <div className={`
      ...
      `}>
        <div className="flex items-start gap-4">
          {/* Icon */}
          <div className={`text-3xl flex-shrink-0 ${usedToday ? 'grayscale opacity-50' : ''}`}>
            {spread.icon}
          </div>
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <h3 className={`font-bold text-lg ${usedToday ? 'text-slate-400' : 'text-white'}`}>
                {spread.name}
              </h3>
              {usedToday && (
            ...
            <p className={`text-sm leading-relaxed mb-2 ${usedToday ? 'text-slate-500' : 'text-slate-400'}`}>
              {spread.description}
            </p>
