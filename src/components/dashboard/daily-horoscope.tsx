...
export function DailyHoroscope({ horoscope, isPersonalized = true }: DailyHoroscopeProps) {
  ...
  return (
    <div className="bg-indigo-950/30 rounded-2xl overflow-hidden mb-8">
      {/* Sample data banner */}
      {!isPersonalized && (
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-indigo-200/50 text-sm">{formatDate(horoscope.date)}</p>
            </div>
          </div>
          <div className={`px-3 py-1.5 rounded-full ${energy.bg}`}>
      ...
      <div className="px-6 py-5">
        ...
        <div className="space-y-2 mb-6">
          {sections.map((section) => (
            <div key={section.id} className="bg-indigo-950/40 rounded-xl overflow-hidden">
              <button
                onClick={() => toggleSection(section.id)}
                className="w-full px-4 py-3 flex items-center justify-between text-left hover:bg-indigo-900/30 transition-colors"
              >
                <div className="flex items-center gap-3">
              ...
              {expandedSection === section.id && (
                <div className="px-4 pb-4">
                  <p className="text-indigo-200/70 text-sm leading-relaxed pl-9">
                    {section.content}
                  </p>
                </div>
              )}
        ...
        <div className="grid grid-cols-2 gap-4 mb-5">
          <div className="bg-indigo-950/40 rounded-xl p-4">
            <div className="text-indigo-300/50 text-xs uppercase tracking-wider mb-1">Lucky Number</div>
            <div className="text-2xl font-semibold text-white">{horoscope.luckyNumber}</div>
          </div>
          <div className="bg-indigo-950/40 rounded-xl p-4">
            <div className="text-indigo-300/50 text-xs uppercase tracking-wider mb-1">Lucky Color</div>
            <div className="flex items-center gap-2">
              <div
